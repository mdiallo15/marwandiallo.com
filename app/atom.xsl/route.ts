// XSL stylesheet for /atom.xml. Same idea as feed.xsl: pretty browser
// preview, transparent to feed readers.
...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="/">
    <html lang="en">
      ...
      <body>
        <header>
          <h1><xsl:value-of select="/atom:feed/atom:title"/> — Atom feed</h1>
          <div class="meta"><xsl:value-of select="/atom:feed/atom:subtitle"/></div>
          <div class="note">
            This is an Atom feed. Subscribe in any feed reader by pasting this
            page's URL.
          </div>
        </header>
        <ul>
        ...
        <footer>
          <a><xsl:attribute name="href"><xsl:value-of select="/atom:feed/atom:link[@rel='alternate']/@href"/></xsl:attribute>
            ← back home
          </a>
        </footer>
      </body>
