...
I went into the registry to settle a disagreement between an agent-based scanner that said the domain controller was clean and a network probe that said it wasn't. The probe was right. The scanner-coverage side of that problem is in [*When 'No Vulnerabilities Found' Means 'We Didn't Look There'*](/writing/no-vulnerabilities-found). What I want to write down here is the shape of the trap that made it possible for the host to look hardened from one angle and unhardened from another, because the same shape shows up on almost every Windows TLS finding I've worked since.
