...
A static CSP analyzer is not a substitute for a real-world security audit. It checks a string against a fixed ruleset. Things it does not do:

- Fetch the page and verify the policy is actually enforced. (The lab's runtime mode does this now.)
...
What it does well is give you a one-minute read on whether the security header you spent half an hour writing is actually doing its job. It is roughly the same category of feedback as ESLint or `cargo clippy`: a fast, deterministic linter for one specific class of risk. The lab now has a runtime mode where you paste any URL, the server fetches the live response headers, and the same ruleset runs against the actual policy on the wire. That closes the gap between the policy you think you ship and the policy you actually ship.
